...
- (void)captureOutput:(AVCaptureOutput*)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection*)connection {
    ...
    try {
        ...
        if(self.isSAMARLabel){
            CGImageRef videoFrameImage1 = [ZXCGImageLuminanceSource createImageFromBuffer:videoFrame];
            /// Colour inversion for DataMatrix
            size_t widthI = CGImageGetWidth(videoFrameImage1);
            size_t heightI = CGImageGetHeight(videoFrameImage1);
            ...
            CGContextDrawImage(context, CGRectMake(0, 0, widthI, heightI),videoFrameImage1);
            ...
            videoFrameImage = CGBitmapContextCreateImage(context);
            
            // clean up
            CGImageRelease(videoFrameImage1);
